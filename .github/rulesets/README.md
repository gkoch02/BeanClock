...
```bash
gh api /repos/gkoch02/BeanClock/rulesets \
  --jq '.[] | select(.name=="Protect main") | .id'
gh api --method PUT /repos/gkoch02/BeanClock/rulesets/<id> \
  --input .github/rulesets/main.json
```

The required status-check contexts must match the unconditional jobs emitted by
`.github/workflows/ci.yml`. A context that never reports will block every merge.
